...
import { EMPTY_TRAINING_FORM, TRAINING_TYPE_OPTIONS, validateTrainingForm, type TrainingSessionFormValues } from "@/lib/training";
...
export default function TreinosNovoClient({ teams }: { teams: Array<{ id: string; name: string; tag: string | null }> }) {
  const router = useRouter();
  const [form, setForm] = useState<TrainingSessionFormValues>(EMPTY_TRAINING_FORM);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  const updateField = (field: keyof TrainingSessionFormValues, value: string) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!response.ok) {
      setError(text || "Não foi possível criar o treino.");
      setSaving(false);
      return;
    }
      ...
      <div className="mx-auto max-w-4xl px-4 py-10 md:px-6">
        <div className="mb-6 flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black">Novo treino</h1>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <label className="block space-y-2 text-sm text-zinc-300">
                ...
                <input value={form.title} onChange={(event) => updateField("title", event.target.value)} className={fieldClass} placeholder="Treino de posicionamento" required />
              </label>
            </div>
            ...
            <label className="block space-y-2 text-sm text-zinc-300">
              ...
              <input value={form.competitive_level} onChange={(event) => updateField("competitive_level", event.target.value)} className={fieldClass} placeholder="Intermediário" />
            </label>
            ...
            <label className="block space-y-2 text-sm text-zinc-300">
              ...
              <input type="number" min="1" value={form.duration_minutes} onChange={(event) => updateField("duration_minutes", event.target.value)} className={fieldClass} placeholder="60" />
            </label>
            ...
            <label className="block space-y-2 text-sm text-zinc-300">
              ...
              <input value={form.region} onChange={(event) => updateField("region", event.target.value)} className={fieldClass} placeholder="Brasil" />
            </label>

            <label className="block space-y-2 text-sm text-zinc-300">
              <span className="font-medium">Estado</span>
              <input value={form.state} onChange={(event) => updateField("state", event.target.value)} className={fieldClass} placeholder="SP" />
            </label>

            <label className="block space-y-2 text-sm text-zinc-300">
              <span className="font-medium">Cidade</span>
              <input value={form.city} onChange={(event) => updateField("city", event.target.value)} className={fieldClass} placeholder="São Paulo" />
            </label>
            ...
            <label className="block space-y-2 text-sm text-zinc-300 md:col-span-2">
              ...
              <textarea value={form.description} onChange={(event) => updateField("description", event.target.value)} rows={5} className={fieldClass} placeholder="Descreva o objetivo, foco e regras do treino." />
            </label>
          </div>
